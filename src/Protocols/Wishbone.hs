...
wishboneSource dfProxy blankDfProxy blankPayload respondAddress statusAddress fifoDepth = Circuit (hideReset circuitFunction) where
  ...
  rightStateMachine brRead = do
    ...
    (currOtpL, currOtpR, numFree, nextRead, nextWrite) <- get -- TODO huh????!
    ...
    pure (brReadAddr, rightOtp) -- TODO why doesnt right otp get changed, also why not just get these values using `get` in `fullStateMachine`?
