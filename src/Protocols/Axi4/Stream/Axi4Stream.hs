...
-- 'Axi4StreamM2S' except for '_tdata'. Used in 'DfLike' implementation.
...
instance (GoodAxi4StreamConfig conf) =>
  DfLike.DfLike   (Reverse (Axi4Stream dom conf userType)) where
...
instance (GoodAxi4StreamConfig conf) =>
  DfLike.ImplicitInfoClass    (Reverse (Axi4Stream dom conf userType)) where
  type ImplicitInfoBwdPayload (Reverse (Axi4Stream dom conf userType)) = Vec (DataWidth conf) Axi4StreamByte

  mapFwd _ () = ()
  mapBwd _ (_, dat) = dat

instance (GoodAxi4StreamConfig conf, NFDataX userType) =>
  DfLike.DfLike    (Axi4Stream dom conf userType) where
...
instance (GoodAxi4StreamConfig conf, NFDataX userType) =>
  DfLike.ImplicitInfoClass (Axi4Stream dom conf userType) where
  type ImplicitInfoFwdPayload (Axi4Stream dom conf userType)
    = Vec (DataWidth conf) Axi4StreamByte
  type ImplicitInfoFwdParam (Axi4Stream dom conf userType)
    = Axi4StreamExtraInfo conf userType
...
instance (GoodAxi4StreamConfig conf, NFDataX userType, KnownDomain dom) =>
  ...
  driveC conf vals
    = withClockResetEnable clockGen resetGen enableGen
    $ DfLike.drive (Proxy, ()) conf (axi4StreamM2SToData <$> vals)
  sampleC conf ckt
    = withClockResetEnable clockGen resetGen enableGen
